...
export function LeadDetailClient({
  ...
  async function handleCopyLeadId() {
    try {
      await navigator.clipboard.writeText(lead.id);
      toastSuccess("Lead ID copied.");
    } catch (error) {
      toastApiError(error, "Unable to copy Lead ID.");
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className={`${badgeBase} ${statusStyles[lead.status] ?? "bg-slate-100 text-slate-700"}`}>
            {lead.status}
          </span>
          ...
          <Button variant="outline" size="sm" onClick={handleCopyLeadId}>
            Copy Lead ID
          </Button>
        </div>
      </div>
